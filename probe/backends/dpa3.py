...
# Prefer ASE if available (covers the full table + synonyms)
try:
    import ase.data as _ase_data
    for _sym, _z in _ase_data.atomic_numbers.items():
        if isinstance(_sym, str) and _sym and _sym[0].isupper():
            _ELEMENT_Z.setdefault(_sym, int(_z))
